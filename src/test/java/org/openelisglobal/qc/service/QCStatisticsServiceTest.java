...
public class QCStatisticsServiceTest {
    ...
    public void testCalculateInitialRunsStatistics_ShouldCalculateMeanAndStdDev() {
        // Arrange — 20 values with known mean=100.0
        ...
        // Act
        statisticsService.calculateInitialRunsStatistics(testControlLotId, 20);

        // Assert — capture the object actually passed to insert() to verify computed
        // values
        ArgumentCaptor<QCStatistics> captor = ArgumentCaptor.forClass(QCStatistics.class);
        verify(statisticsDAO, times(1)).insert(captor.capture());
        QCStatistics computed = captor.getValue();

        assertEquals("Mean should be 100.0", new BigDecimal("100.0"),
                computed.getMean().setScale(1, RoundingMode.HALF_UP));
        assertTrue("Standard deviation should be positive",
                computed.getStandardDeviation().compareTo(BigDecimal.ZERO) > 0);
        assertEquals("Should use all 20 values", Integer.valueOf(20), computed.getNumValues());
        assertEquals("Calculation method should be INITIAL_RUNS", "INITIAL_RUNS", computed.getCalculationMethod());
    }
    ...
    public void testCalculateRollingStatistics_ShouldUseMostRecentResults() {
        // Arrange — 10 values with known mean=99.9
        ...
        // Act
        statisticsService.calculateRollingStatistics(testControlLotId, 10);
        ...
        assertEquals("Mean should be 99.9", new BigDecimal("99.9"),
                computed.getMean().setScale(1, RoundingMode.HALF_UP));
        assertTrue("Standard deviation should be positive",
                computed.getStandardDeviation().compareTo(BigDecimal.ZERO) > 0);
        assertEquals("Should use 10 most recent values", Integer.valueOf(10), computed.getNumValues());
        assertEquals("Calculation method should be ROLLING", "ROLLING", computed.getCalculationMethod());
    }

    // testGetLatestStatistics removed: service method is a pure DAO passthrough
    // (return statisticsDAO.findLatestByControlLot()), so the test only asserted
    // that the mock returned what it was told to return.
    ...
    public void testCalculateStandardDeviation_ShouldReturnCorrectSD() {
        ...
        // Assert — SD of {95, 100, 105} = 5.0 (sample std dev)
        assertNotNull("Standard deviation should not be null", stdDev);
        assertEquals("Standard deviation should be 5.0", new BigDecimal("5.0"),
                stdDev.setScale(1, RoundingMode.HALF_UP));
    }
